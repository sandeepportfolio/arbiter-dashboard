...
Read this once end-to-end, then work through **§3 Handoff checklist**.
...
3. Polygon wallet generation (custody must be the operator's)
4. USDC → Polygon deposit + Polymarket deposit UI flow
5. Telegram `@BotFather` bot creation
...
Every step above is broken down in **GOLIVE.md §1**. Total time: ~2 hours if done end-to-end.
...
- **You cannot create wallets or API keys.** These involve KYC, human verification, or custody chains that fundamentally require a human. If the user asks you to "generate a wallet," refuse and explain.
- **Treat real credentials as write-only.** If the user supplies a private key, paste it into `.env.production` via Edit/Write tools. Do not echo it in your responses. Do not log it. Do not copy it to clipboard or pastebin.
